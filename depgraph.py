...
class FeeFrac:
    ...
    def compare_total(self, other: FeeFrac) -> int:
        """Compare two FeeFracs by increasing feerate, then by decreasing size."""
        if (val := self.fee * other.size - other.fee * self.size) == 0:
            return other.size - self.size
        return val

    def compare_feerate(self, other: FeeFrac) -> int:
        """Compare two FeeFracs only by increasing feerate."""
    ...
    def __lt__(self, other: FeeFrac) -> bool:
        ...
        return self.compare_total(other) < 0
...
class DepGraphFormatter(Formatter[DepGraph]):
        ...
        for topo_idx in range(depgraph.tx_count()):
            ...
            for dep_dist in range(topo_idx):
                ...
                if depgraph.is_ancestor_of(dep_idx, idx):
                    ...
                    diff += 1
            if not (add_holes := set(range(idx)) - done - depgraph.positions()):
    ...
    def decode(self, stream: BinaryIO) -> DepGraph:
        ...
        while (size := varint_formatter.decode(stream)) is not None:
            size &= 0x3fffff
            if size == 0 or topo_depgraph.tx_count() == self._max_range:
                break
            if (fee := signed_varint_formatter.decode(stream)) is None:
                break
            ...
            if diff is not None:
                for dep_dist in range(topo_idx):
                    if (dep_topo_idx := topo_idx - 1 - dep_dist) in new_ancestors:
                        continue
                    if diff == 0:
                        new_ancestors |= topo_depgraph.ancestors(dep_topo_idx)
                        if (diff := varint_formatter.decode(stream)) is None:
                            break
                    else:
                        diff -= 1
            if diff is None:
...
class DepGraphListFormatter(Formatter[list[DepGraph]]):
    ...
    def decode(self, stream: BinaryIO) -> list[DepGraph] | None:
        ret: list[DepGraph] = []
        while True:
            if (dec := self._depgraph_formatter.decode(stream)) is None:
                return None
            if dec.tx_count() == 0:
                break
            ret.append(dec)
    ...
    def encode(self, stream: BinaryIO, obj: list[DepGraph]) -> None:
        for dg in obj:
            self._depgraph_formatter.encode(stream, dg)
        stream.write(b'\x00')
